def genuine_data_generate():
	import random	

	number_of_unique_IMSI = 1000
	IMSI_list = []
	initialIMSI = 100025421542369
	temp = initialIMSI
	...
	number_of_unique_IMEI = 1000
	IMEI_list = []
	initialIMEI = 300025421542369
	temp = initialIMEI
	...
	for i in range(0,len(imei_imsi_pair)) :
		...
		for j in range (0,activity[i]):
			...
			duration = random.randint(100,500)									#For Fake SIms this duration will be less so the limit will be low
			endtimeforcdr = starttimeforcdr + duration
			starttime = endtimeforcdr + random.randint(100,1000)				#For Fake sims this limit will be very less as the idle duration between two calls
